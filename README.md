...
A dificuldade de manter a consistência em pequenas rotinas diárias (como tomar remédios, exercícios ou estudos) pode prejudicar a saúde e a organização pessoal.

**Solução Proposta:**

Uma aplicação simples de interface via linha de comando (CLI) que permite registrar e visualizar o acompanhamento de hábitos diários, definidos pelo usuário, funcionando como um *tracker* minimalista. A aplicação visa garantir uma das partes mais difíceis que é a facilidade e constância de registros. Com o histórico em mãos, a aplicação dá autonomia para que os dados JSON sejam utilizados pelo próprio usuário para posteriores criações de dataframes e gráficos visuais.  

**Público-Alvo:** 
Estudantes, profissionais e qualquer pessoa que possua mínima familiaridade com CLI's que precise de uma ferramenta rápida e sem distrações visuais para monitorar sua rotina. 
...
```
**2. Configurando seus hábitos**

Edite o arquivo `habits.txt` na raiz do projeto. Você pode usar # para comentar:

```markdown
# Meus Hábitos de 2026
Estudar Python
Treino de força
Beber 2L de água
```
